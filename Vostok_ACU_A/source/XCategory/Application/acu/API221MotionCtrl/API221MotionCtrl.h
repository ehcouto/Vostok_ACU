...
typedef enum API221_FAILURE_FLAGS_ENUM
{
    ...
    API221_FF_IC_IMPLAUSIBLE_MIN    = 5,            //!< Safety input capture plausibility check failed
    API221_FF_IC_IMPLAUSIBLE_MAX    = 6,            //!< Safety input capture plausibility check for maximum speed change failed
    API221_FF_INVERTER_OVER_TEMP    = 7,            //!< Module over temperature
    API221_FF_INITIALIZATION_ERROR  = 8,            //!< Initialization phase failed
    API221_FF_STATOR_OVER_TEMP      = 9,            //!< Stator temperature exceeded the limit and will prevent start for 30 seconds
    ...
    API221_FF_CLASS_B_FORCE2STOP    = 18,           //!< Class B forced the motor to stop
    API221_FF_INT_DISABLED          = 19,           //!< Interrupt was disabled
    API221_FF_OBSERVER_FAILURE      = 20            //!< Observer was unable to observe
...
typedef enum API221_MOTOR_EVENT_ENUM
{
    ...
    API221_EVENT_REMOVED_ERROR      = 5,            //!< One or more errors is no longer present
...
typedef struct API221_MOTOR_ID_STRUCT
{
    ...
    uint8 Motor_Index;                              //!< The 0-based index of the motor on the motor control node.
...
void API221MotionCtrl__FeedbackParser(REVEAL_MSG_TYPE* fbk);
    ...
    // DEPRECATED Functions. Will be removed in a future release.
